...
module _ {A}(ACTA : Act A) where
  ...
  record ActIden : Set where
    field
      ...
      idaHit : forall {G M}(i : <> <- G) -> hit i (ida {G}{M}) == # i
    ...
    idaNeu : forall {M G}(n : Term M G ess syn) -> act n (ida {G}) == essl n
...
module _ {F B C : Bwd Nat -> Nat -> Nat -> Set}
  (of : forall {N} -> ObjAct N (F N))
  (ob : forall {N} -> ObjAct N (B N))
  (oc : forall {N} -> ObjAct N (C N))
  where
  open Act
  open ActCompo
  open POLYOBJACT
  ...
  record ObjComp : Set where
    field
      ...
      hitth : forall {G0 M1 G1 M2 G2}(i : <> <- G0)
        ...
        act (objAct _ ob) (objHit _ of i f) (ph , b) ==
          objHit _ oc i (coth f ph b)
...
module _ {A B C D E}
  ...
  {-
  compoLemma : forall {M0 G0 MG1 MG2 MG3}
    (a : A (M0 , G0) MG1)(b : B MG1 MG3)(d : D (M0 , G0) MG2)(e : E MG2 MG3) ->
    (co ABC a b == co DEC d e) ->
    forall {z}{X : Nat}(tz : All (\ _ -> Term M0 G0 lib z) X) ->
    all (\ t -> act ACTB t b) (all (\ t -> act ACTA t a) tz) ==
    all (\ t -> act ACTE t e) (all (\ t -> act ACTD t d) tz)
  compoLemma a b d e q tz = 
    all (\ t -> act ACTB t b) (all (\ t -> act ACTA t a) tz)
      =< allCo _ _ _ (\ t ->
        act ACTC t (co DEC d e) =< act ACTC t $= q ]=
        act ACTC t (co ABC a b) =< coLib ABC t a b ]=
        act ACTB (act ACTA t a) b [QED]) _ ]=
    all (\ t -> act ACTC t (co DEC d e)) tz
      =[ allCo _ _ _ (\ t -> sym (coLib DEC t d e)) _ >=
...
module _ {A : Bwd Nat -> Nat -> Nat -> Set}(OA : forall {N} -> ObjAct N (A N)) where
  ...
  record SbstThen : Set where
    field
      hitthZero : forall {M G D}(al : A M G D) -> objHit (oe su) (objWkn al) == # (oe su)
    ...
    hitth compSbst i sg ph al = sym (top $= selectAll i _ sg)
...
{-
    module _ (ASS : ObjComp OA OBJSBST OBJSBST) where
      open ActCompo
    
      AINSTINST : ActCompo (objAct _ OA) INST INST
      co AINSTINST (th , al) (xi , sg) = select th xi , coth ASS al {!!} sg
      hitCo AINSTINST = {!!}
      metCo AINSTINST = {!!}
      wknCo AINSTINST = {!!}
-}
