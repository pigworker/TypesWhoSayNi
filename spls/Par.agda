...
pam (!!! a) (! a') with atomEq? a a'
pam (!!! a) (! a') | #0 , r = #0 , \ { (!!! a) -> r refl }
pam (!!! a) (! .a) | #1 , refl = #1 , !!! a
pam (!!! a) (_ & _) = #0 , \ ()
pam (!!! a) (\\ _) = #0 , \ ()
pam (!!! a) (thnk _) = #0 , \ ()
...
pam (p &&& q) (s & t) | #0 , pr | qb , qr = #0 , \ { (py &&& _) -> pr py }
pam (p &&& q) (s & t) | #1 , pr | #0 , qr = #0 , \ { (_ &&& qy) -> qr qy }
pam (p &&& q) (s & t) | #1 , pr | #1 , qr = #1 , pr &&& qr
pam (p &&& q) (\\ _) = #0 , \ ()
pam (p &&& q) (thnk t) = #0 , \ ()
pam (p &&& q) (_ ?- _) = #0 , \ ()
pam (\\\ q) (! _) = #0 , \ ()
...
pam (\\\ q) (thnk _) = #0 , \ ()
pam (\\\ q) (_ ?- _) = #0 , \ ()
pam (??? _ th) t with thickR? th t
pam (??? _ th) t | #0 , z = #0 , \ { (??? x) -> z (_ , x) }
pam (??? _ th) t | #1 , s , st = #1 , ??? st
...
module PARALLEL (TH : TypeTheory) where
  ...
  reflParP {p = !!! a} (!!! .a) = !!! a
  reflParP {p = p &&& q} (ss &&& ts) = reflParP ss &&& reflParP ts
  reflParP {p = \\\ q} (\\\ ts) = \\\ reflParP ts
  reflParP {p = ??? _ th} (??? t) = ??? (reflPar t)
  ...
  parRedsP {p = !!! a} (!!! .a) = []
  parRedsP {p = p &&& q} (ss &&& ts) =
    star (_& _) car (parRedsP ss) *+ star (_ &_) cdr (parRedsP ts)
  parRedsP {p = \\\ q} (\\\ ts) = star (\\_) abst (parRedsP ts)
  parRedsP {p = ??? _ th} (??? t) =
  ...
  parThick ((t0 :: T0) $ s0) th t = help _ _ _ refl refl refl t where
    ...
    ... | ts2 , ts' , refl | Ts2 , Ts' , refl | ss2 , ss' , refl
      = _ , beta x ts' Ts' ss' , sym (instThinLemma (redTerm R :: redType R) [] ((ts2 &&& Ts2) &&& ss2) th)
  ...
  parPThick (!!! a) (!!! .a) th (!!! .a) = !!! a , !!! a , refl
  parPThick (p &&& q) (pi0 &&& chi0) th (pi &&& chi) with parPThick p pi0 th pi | parPThick q chi0 th chi
  ... | pi1 , pi' , refl | chi1 , chi' , refl = pi1 &&& chi1 , pi' &&& chi' , refl
  parPThick (\\\ q) (\\\ chi0) th (\\\ chi) with parPThick q chi0 th chi
  ... | chi1 , chi' , refl = \\\ chi1 , \\\ chi' , refl
  parPThick (??? _ {de} ph) (??? t0) th (??? t) with parThick t0 (th ^+ oi {S = de}) t
  ... | t1 , t' , refl = _ , ??? t' , refl

  parStan : forall {M ga de}(p : Pat de)(pi : Env M (ga ,P p)) r -> (p %P pi) => r ->
    Sg _ \ chi -> r == (p %P chi) * pi =P> chi
  parStan (!!! a) (!!! .a) .(! a) (atom .a) = !!! a , refl , !!! a
  parStan (p &&& q) (pip &&& piq) .(_ & _) (cons px qx)
    with parStan p pip _ px | parStan q piq _ qx
  ... | chip , refl , pipg | chiq , refl , piqg
      = chip &&& chiq , refl , pipg &&& piqg
  parStan (\\\ q) (\\\ pi) .(\\ _) (abst x)
    with parStan q pi _ x
  ... | chi , refl , pig = \\\ chi , refl , \\\ pig
  parStan (??? _ th) (??? t) r x with parThick t (oi ^+ th) x
  ... | t' , y , refl = ??? t' , refl , ??? y
  ...
  parThin (beta {R} x {ts0} {ts1} {Ts0} {Ts1} {ss0} {ss1} ts Ts ss) th
    rewrite plugThinLemma (betaIntro R) ts0 th
          ...
          | instThinLemma (redTerm R) [] ((ts1 &&& Ts1) &&& ss1) th
          | instThinLemma (redType R) [] ((ts1 &&& Ts1) &&& ss1) th
  ...
  parpThin (!!! a) (!!! .a) th = !!! a
  parpThin (p &&& q) (ss &&& ts) th = parpThin p ss th &&& parpThin q ts th
  parpThin (\\\ q) (\\\ ts) th = \\\ parpThin q ts th
  parpThin (??? _ {de} ph) (??? t) th = ??? (parThin t (th ^+ oi {S = de}))
  ...
  parnSbst (beta {R} x {ts0} {ts1} {Ts0} {Ts1} {ss0} {ss1} ts Ts ss) {sg0} {sg1} sg
    rewrite plugSbstLemma0 (betaIntro R) ts0 sg0
          ...
          | instSbstLemma0 (redTerm R) [] ((ts1 &&& Ts1) &&& ss1) sg1
          | instSbstLemma0 (redType R) [] ((ts1 &&& Ts1) &&& ss1) sg1
  ...
  parpSbst (!!! a) (!!! .a) sg = !!! a
  parpSbst (p &&& q) (ss &&& ts) sg = parpSbst p ss sg &&& parpSbst q ts sg
  parpSbst (\\\ q) (\\\ ts) sg = \\\ parpSbst q ts sg
  parpSbst (??? _ {de} th) (??? t) sg
    = ??? (parSbst t (parCat (parzThin sg _) (parVarz {ga = de} _)))

  parProje : forall {M ga de de'}{p : Pat de'}(x : de <P- p){pi pi' : Env M (ga ,P p)} ->
    pi =P> pi' -> proje ga x pi => proje ga x pi'
  parProje ??? (??? x) = x
  parProje (car x) (pi &&& _) = parProje x pi
  parProje (cdr x) (_ &&& pi) = parProje x pi
  parProje (\\\ x) (\\\ pi) = parProje x pi
  ...
  betaInv : forall {b}(x : b <- computation){M ga}
            ...
            ((pit' : Env M (ga ,P betaIntro b))(piT' : Env M (ga ,P betaType b))(pis' : Env M (ga ,P betaElim b)) ->
             pit =P> pit' -> piT =P> piT' -> pis =P> pis' -> P ((redTerm b :: redType b) % ([] , (pit' &&& piT') &&& pis'))) ->            
  ...
  betaInv {b} x pit piT pis qt qT qs pno pgo .(_ :: _) (beta {b'} y {ts} {Ts = Ts} {ss = ss} pitr piTr pisr)
    with noAmbiguity _ (\ x y r -> sym~~ _ _ r)
           ...
           (matchU ((betaIntro b &&& betaType b) &&& betaElim b) ((pit &&& piT) &&& pis)
                   ((betaIntro b' &&& betaType b') &&& betaElim b') ((ts &&& Ts) &&& ss)
                   (_&_ $= (_&_ $= qt =$= qT) =$= qs))
           x y
  ... | refl , refl
  ...
  go : forall {M ga ga'}{t T s : Term M ga lib chk}{t' T' s' : Term M ga' lib chk}{th : ga <= ga'} ->
    ...
    (forall {b}(x : b <- computation) ->
      Match (ga ,P (betaIntro b &&& betaType b) &&& betaElim b) ((t & T) & s) -> b <- bz) ->
  ...
  ... | dt , rt , ut | dT , rT , uT | ds , rs , us = (dt :: dT) $ ds , elim (radi rt rT) rs , 
    \ { _ (elim (radi xt xT) xs) -> elim (radi (ut _ xt) (uT _ xT)) (us _ xs)
      ; _ (beta {R} x {ts = ts}{Ts = Ts}{ss = ss} _ _ _) ->
        naughty (busted (bc x (match ((betaIntro R &&& betaType R) &&& betaElim R) ((ts &&& Ts) &&& ss)))) }
  go {ga = ga} {t = t0} {T0} {s0} t T s (bz -, b) be bc
    with pam (ga ,P (betaIntro b &&& betaType b) &&& betaElim b) ((t0 & T0) & s0)
  go {ga = ga} {t = t0} {T0} {s0} t T s (bz -, b) be bc | #0 , n =
    go t T s bz ((oi no) -< be) (help computation bc) where
      help : forall cz ->
        (forall {b'}(x : b' <- cz) ->
          Match (ga ,P (betaIntro b' &&& betaType b') &&& betaElim b') ((t0 & T0) & s0) -> b' <- (bz -, b)) ->
        forall {b'}(x : b' <- cz) ->
          Match (ga ,P (betaIntro b' &&& betaType b') &&& betaElim b') ((t0 & T0) & s0) -> b' <- bz
      help ._ cc x m with cc x m
      help ._ cc x m | w no = w
      help ._ cc x m | w su = naughty (n m)
  go {ga = ga} {t = t0} {T0} {s0} t T s (bz -, b) be bc | #1 , (tm &&& Tm) &&& sm
    with devo t (betaIntro b) tm | devo T (betaType b) Tm | devo s (betaElim b) sm
  ... | pitt , refl , pidt , pitdt , pitu
      | pitT , refl , pidT , pitdT , piTu
      | pits , refl , pids , pitds , pisu
      = ((redTerm b :: redType b) % ([] , (pidt &&& pidT) &&& pids))
      ...
      , betaInv ((oe su) -< be) pitt pitT pits refl refl refl
        ...
        \ pit' piT' pis' rt rT rs -> parStab (redTerm b :: redType b) ((pitu _ rt &&& piTu _ rT) &&& pisu _ rs)
  devo t (??? _ th) (??? x) with dev (x +t t)
  devo t (??? _ th) (??? {t = t0} x) | d , td , du = ??? t0 , nihtR x , ??? d , ??? td , \ { _ (??? y) -> ??? (du  _ y) }
  devo (essl (atom .a)) (!!! a) (!!! .a) = !!! a , refl , !!! a , !!! a , \ { _ (!!! a) -> !!! a }
  devo (essl (cons s t)) (p &&& q) (ps &&& qt) with devo s p ps | devo t q qt
  ... | spi , refl , schi , sr , us | tpi , refl , tchi , tr , ut
    = spi &&& tpi , refl , schi &&& tchi , sr &&& tr
    , \ { _ (ys &&& yt) -> us _ ys &&& ut _ yt }
  devo (essl (abst t)) (\\\ q) (\\\ qt) with devo t q qt
  ... | tpi , refl , tchi , tr , ut
      = \\\ tpi , refl , \\\ tchi , \\\ tr
      , \ { _ (\\\ yt) -> \\\ (ut _ yt) }
