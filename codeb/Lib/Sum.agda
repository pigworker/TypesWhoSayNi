...
module _ {l}{S S' T T' : Set l}(SR : S -> S' -> Set l)(TR : T -> T' -> Set l)
 where

 data _+R_ : (S + T) -> (S' + T') -> Set l where
   inl : forall {s s'} -> SR s s' -> _+R_ (inl s) (inl s')
   inr : forall {t t'} -> TR t t' -> _+R_ (inr t) (inr t')
