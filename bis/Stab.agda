...
module _ where
  ...
  redThin (thnk {e = e} n) th rewrite thinThunkLemma e th = thnk (redThin n th)
  ...
  redThin (beta {R} x ts Ts ss) th
    rewrite plugThinLemma (betaIntro R) ts (refl , th)
          | plugThinLemma (betaType R) Ts (refl , th)
          | plugThinLemma (betaElim R) ss (refl , th)
          | instThinLemma (redTerm R) (_ , refl , [] , cons (cons ts Ts) ss) th
          | instThinLemma (redType R) (_ , refl , [] , cons (cons ts Ts) ss) th
          = beta {R = R} x _ _ _
