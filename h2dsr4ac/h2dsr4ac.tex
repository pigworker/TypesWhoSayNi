...
Let us begin at the beginning, which is to introduce a notion of multi-sorted syntax with binding, in the style of universal algebra. The whole setup is parametrised over some $\Sort\hab\Set$ of \emph{term} sorts, to be defined by mutual induction. We shall need to explain how to build terms from collections of subterms. So let us introduce a notion of \emph{arity}, $\Ar\Sort$, so that we can also have a parameter specifying the \emph{constructors} $\Ctor\hab \Sort\to\Ar\Sort \to\Set$, such that $\Ctor\:S\:T$ tells us which constructor tags make a term of sort $S$ with arity $T$. All $\Ar$ does is close $\Sort$ under unit, pairing and abstraction:
\[
\Rule{S\hab\Sort}
     {S\hab\Ar\Sort}
\qquad
\Axiom{\SUn\hab\Ar\Sort}
\qquad
\Rule{T,U\hab\Ar\Sort}
     {T\SPr U\hab\Ar\Sort}
\qquad
\Rule{T,U\hab\Ar\Sort}
     {T\SBi U\hab\Ar\Sort}
...
\]
Were we to introduce a \emph{de Bruijn} syntax, we should have terms
\[
\Rule{T\hab\Ar\Sort\quad \Gamma\hab\Bwd\Sort}
     {\dB T\Gamma \hab \Set}
\]
...
\end{array}\]
where $S\lar\Gamma$ is the type of locations of $S$ in $\Gamma$, i.e., glorified numbers guaranteed to be in bounds and to index the intended sort.

However, my aim is to be relevant, for a change. We shall have \emph{the} variable, but constructors are as before:
\[
...
\]
Unit and pairing are sensitive to scope. I shall detail $\thi$ and $\cov$ shortly, but they allow us to ensure that the left scope $\Gamma_l$ and the right scope $\Gamma_r$ collectively cover the whole scope $\Gamma$.
\[
...
\]
Binding comes in vacuous and relevant variants, so that the unused variable never comes \emph{into} scope:
\[
