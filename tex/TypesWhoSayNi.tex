...
\begin{example}[Martin-L\"of's 1971 theory: syntax and $\beta$-reduction]
  ...
  $\beta$-rule is
\[
  \ra{\la xt}{\PI SxT}\:s \leadsto \ra{t\fsl\ra sS}{T\fsl\ra sS}
\]
\end{example}
...
\[\ra{\cn st}{\SG SxT}\:\car \leadsto \ra sS \qquad
  \ra{\cn st}{\SG SxT}\:\cdr \leadsto \ra t{T\fsl\ra sS}
\]
...
\begin{definition}[action of a thinning]
  If $\theta:\gamma\thin \delta$, it has a quadrant-preserving action,
...
is the target scope of $\theta$.

By contrast, thinnings act \emph{contravariantly} on vectors of any kind.
Viewed left-to-right, thinnings represent the injection of one scope
...
length of the $\sel\theta{\vec x}$ is given by the source scope
of $\theta$.
...
\begin{definition}[$\gamma$-context, actions on contexts, global
  ...
  \item If $\Gamma$ is a valid $\gamma$-context and $\Gamma\vdash\ty
    S_\gamma$, then $\wka\Gamma,x\!:\!S_\gamma$ is a valid $\gamma,x$ context.
  \end{itemize}
\end{definition}
...
\begin{definition}[context extension, type lookup]
  \[\textsc{extend}\;
    ...
    \qquad
    \textsc{lookup}\;\Axiom{\Gamma\vdash x\in x\Gamma}
  \]
\end{definition}
...
server to guarantee that $\Gamma\vdash \ty{x\Gamma}$, provided we can
...
\begin{definition}[contextual closure of reduction]
\[
  \Rule{s\leadsto s'}
...
$\ra tT\:s$ as redexes. More particularly, if every well typed radical
...
\end{array}\]
\[
  \Axiom{\ra{\la xt}{\PI SxT}\:s \leadsto \ra{t\fsl\ra sS}{T\fsl\ra sS}}
\]
\end{definition}
...
\[
  \ra{\la xt}{\PI SxT}\:s \leadsto \ra{\;\;?\;\;}{T\fsl\ra sS}
\]
...
\[\begin{array}{lcl}
  \ra{\cn st}{\SG SxT}\:\car&\leadsto& \ra{\;\;?_\car\;\;}S \\
  \ra{\cn st}{\SG SxT}\:\cdr&\leadsto& \ra{\;\;?_\cdr\;\;}{T\fsl\ra{\cn st}{\SG SxT}\:\car}
\end{array}\]
...
\[
  \Rule{T\fsl\ra{\cn st}{\SG SxT}\:\car\leadsto T\fsl\ra sS\quad T\fsl\ra sS\ni t}
...
$p$ is given by the action, $p\pi$, for some $p$-environment $\pi$. The grammar of
...
guarantee that
\[
  (p\pi)\theta = p(\pi\theta)\qquad
  (p\pi)\sigma = p(\pi\sigma)
\]
...
\begin{dogma}[stability of pattern matching under reduction]
  Consider pattern $p$. If $t=p\pi$ for some $p$-environment, $\pi$,
  and $t\leadsto t'$, then there must exist some $\pi'$ such that
  $t'=p\pi'$ and $\pi\leadsto\pi'$, where the latter is the
